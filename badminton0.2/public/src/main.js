...
const BACKGROUND_ASSETS = {
  default: '/assets/arena-panorama-v2.png',
  olympics: '/assets/arena-panorama-v2.png',
  worlds: '/assets/arena-panorama-v2.png',
  'all-england': '/assets/arena-panorama-v2.png',
  'world-tour': '/assets/arena-panorama-v2.png',
};

const textureLoader = new THREE.TextureLoader();
const PLAYER_SPRITE_FRAMES = 6;
...
function buildCourt() {
  ...
  const surroundTexture = textureLoader.load('/assets/rubber-floor-navy-v2.png', (texture) => {
    texture.colorSpace = THREE.SRGBColorSpace;
  ...
  const surroundMat = new THREE.MeshStandardMaterial({ map: surroundTexture, color: 0xa9c4db, roughness: 0.94, metalness: 0.01 });
  ...
  const netTexture = textureLoader.load('/assets/net-realistic-v2.png', (texture) => {
    texture.colorSpace = THREE.SRGBColorSpace;
    texture.anisotropy = Math.min(8, renderer.capabilities.getMaxAnisotropy());
  });
...
function createAvatar() {
  ...
  const texture = textureLoader.load('/assets/player-sprites-v2.png', (loadedTexture) => {
    loadedTexture.colorSpace = THREE.SRGBColorSpace;
  });
  texture.repeat.set(1 / PLAYER_SPRITE_FRAMES, 1);
  ...
  sprite.scale.set(1.62, 2.15, 1);
  sprite.renderOrder = 2;
  group.add(sprite);
  group.userData.sprite = sprite;
  group.userData.texture = texture;
  group.userData.frame = -1;
...
function createFirstPersonRacket() {
  const group = new THREE.Group();
  const texture = textureLoader.load('/assets/racket-realistic-v2.png', (loadedTexture) => {
    loadedTexture.colorSpace = THREE.SRGBColorSpace;
  });
  ...
  const racket = new THREE.Mesh(new THREE.PlaneGeometry(0.56, 0.86), material);
  racket.renderOrder = 4;
  group.add(racket);
  group.position.set(0.5, -0.36, -0.88);
  group.rotation.set(-0.08, 0.1, -0.08);
  return group;
}
...
function bindUi() {
  ...
  dom.instructionsBtn.addEventListener('click', () => showOnly(dom.instructionsPanel));
...
function aiHit(options = {}) {
  ...
  launchToTarget(seat, target, options.forceServe ? 1.05 : rand(0.68, 0.98), {
...
function pickAiServeTarget(seat) {
  ...
  const x = targetSide * rand(1.0, 1.7);
  const z = seat === 'A'
    ? rand(3.8, 4.8)
    : rand(-4.8, -3.8);
...
function launchToTarget(seat, target, flightTime, options = {}) {
  ...
  if (options.isServe) ensureNetClearance(start, velocity, seat === 'A' ? 1 : -1);
...
function updateMeshes(dt) {
  ...
  racketGroup.rotation.z = -0.08 - Math.sin((swing / 210) * Math.PI) * 0.5 * smashScale;
  racketGroup.rotation.x = -0.08 + Math.sin((swing / 210) * Math.PI) * 0.3 * smashScale;
}
...
function placeAvatar(avatar, player, seat) {
  ...
  poseSpriteAvatar(avatar, player, seat);
}

function poseSpriteAvatar(avatar, player, seat) {
  const now = performance.now();
  const swingRemaining = Math.max(0, (player.swingUntil || 0) - now);
  let frame = 0;
  if (swingRemaining > 0) {
    const swingRatio = swingRemaining / 280;
    frame = !player.grounded ? 5 : swingRatio > 0.48 ? 3 : 4;
  } else if (player.moving) {
    frame = Math.floor(now / 120) % 2 === 0 ? 1 : 2;
  } else if (!player.grounded) {
    frame = 5;
  }

  if (frame !== avatar.userData.frame) {
    avatar.userData.texture.offset.x = frame / PLAYER_SPRITE_FRAMES;
    avatar.userData.frame = frame;
  }
  avatar.userData.sprite.material.rotation = seat === 'A' ? 0.015 : -0.015;
}
...
function showOnly(screen) {
  [dom.menu, dom.singlePanel, dom.multiPanel, dom.tournamentPanel, dom.historyPanel, dom.instructionsPanel, dom.endScreen, dom.pauseScreen]
...
function showGameHud() {
  [dom.menu, dom.singlePanel, dom.multiPanel, dom.tournamentPanel, dom.historyPanel, dom.instructionsPanel]
    .forEach((item) => item.classList.add('hidden'));
...
function goMenu() {
  ...
  dom.instructionsPanel.classList.add('hidden');
}
